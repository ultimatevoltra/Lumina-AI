...
import { Sparkles, LogOut, MoreHorizontal, Zap, History, Info, Mail } from "lucide-react";
...
export function Navbar({ onOpenLogin, onOpenSignup }: NavbarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 border-b border-border/50 bg-background/80 backdrop-blur-md">
          ...
          <div className="relative">
            <button
              ...
              className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                MORE_LINKS.some((l) => l.href === location)
                  ? "text-foreground"
                  : "text-muted-foreground hover:text-foreground hover:bg-white/5"
              }`}
              ...
              {moreOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute top-full right-0 mt-2 w-44 glass-card rounded-xl border border-border/50 shadow-2xl z-20 overflow-hidden"
                  >
                    {MORE_LINKS.map((link) => {
      ...
      {/* Points info tooltip for free users */}
      {!user && typeof pointsBalance === "number" && isFinite(pointsBalance) && pointsBalance < PHOTO_POINT_COST * 5 && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="hidden sm:block absolute top-full right-4 mt-2 px-3 py-2 glass-card rounded-xl text-xs text-muted-foreground border border-destructive/20 z-30"
        >
